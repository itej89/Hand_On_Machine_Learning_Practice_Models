...
class fashion_mnist_model :
    ...
    def Build_Auto_Encoder_PCA_UnderComplete(self):
        ...
        from KLDivergenceRegularizer import KLDivergenceRegularizer
        ...
        encoder.add(keras.layers.Dense(300, activation="sigmoid", activity_regularizer=kld_reg))

        decoder = keras.models.Sequential(name= "decoder")
        decoder.add(keras.layers.Dense(100, activation="selu", input_shape=[300]))
    ...
    def model_reconstruct(self, checkpoint_path, test_x,  n_images=5):
        from tensorflow.keras.models import load_model
        from KLDivergenceRegularizer import KLDivergenceRegularizer
        model = load_model(checkpoint_path, custom_objects={"KLDivergenceRegularizer": KLDivergenceRegularizer})
