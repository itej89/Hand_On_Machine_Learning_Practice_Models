...
class fashion_mnist_model :
    ...
    def Build_Auto_Encoder_PCA_UnderComplete(self):
        ...
        encoder.add(keras.layers.Reshape([28, 28, 1], input_shape=[28, 28]))
        encoder.add(keras.layers.Conv2D(16, kernel_size=3, padding="same" ,activation="selu"))
        encoder.add(keras.layers.MaxPool2D(pool_size=2))
        encoder.add(keras.layers.Conv2D(32, kernel_size=3, padding="same" ,activation="selu"))
        encoder.add(keras.layers.MaxPool2D(pool_size=2))
        encoder.add(keras.layers.Conv2D(64, kernel_size=3, padding="same" ,activation="selu"))
        encoder.add(keras.layers.MaxPool2D(pool_size=2))
        

        decoder = keras.models.Sequential(name= "decoder")
        decoder.add(keras.layers.Conv2DTranspose(32, kernel_size=3, 
            padding="valid", strides=2, activation="selu", input_shape=[3,3,64]))
        decoder.add(keras.layers.Conv2DTranspose(16, kernel_size=3, 
            padding="same", strides=2, activation="selu"))
        decoder.add(keras.layers.Conv2DTranspose(1, kernel_size=3, 
    ...
    def model_DimensionalityReduction(self, checkpoint_path, test):
        ...
        #Extract encoder and decoder mnodels
...
Total_Epochs = 500
Batch_Size = 32
Initiate_at_Epoch = 81
Initiate_at_Loss = 0.25
