...
class fashion_mnist_model :
    ...
    def Build_Auto_Encoder_PCA_UnderComplete(self):
        ...
        encoder.add(keras.layers.LSTM(100, return_sequences=True, input_shape=[None, 28]))
        encoder.add(keras.layers.LSTM(30))
        

        decoder = keras.models.Sequential(name= "decoder")
        decoder.add(keras.layers.RepeatVector(28, input_shape=[30]))
        decoder.add(keras.layers.LSTM(100, return_sequences=True))
        decoder.add(keras.layers.TimeDistributed(keras.layers.Dense(28, activation="sigmoid")))
...
Total_Epochs = 500
Batch_Size = 32
Initiate_at_Epoch = 15
Initiate_at_Loss = 0.27
