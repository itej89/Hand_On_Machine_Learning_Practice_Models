...
class fashion_mnist_model :
    ...
    def Build_Keras_Model(self):
        ...
        RegularizedDense = partial(keras.layers.Dense, use_bias=False, kernel_initializer=he_avg_init, kernel_constraint=keras.constraints.max_norm(1.))
    ...
    def fit_model(self, train_X, train_y, X_valid, y_valid, checkpoint_path=None, model_param_path=None, no_epochs=30, batch_size=250, initial_epoch = 0, verbosity=1, get_model = False):
        ...
        ckpt_callback = ModelCheckpoint(filepath=MODEL_PATH_CONSTANTS.GetModelParamPath(), 
                             monitor="val_loss",
                             verbose=1, 
                             save_best_only=True,
                             mode="min")
    ...
    def model_predict_class(self, checkpoint_path, test_x):
        from tensorflow.keras.models import load_model
        from tensorflow import keras
        model = load_model(checkpoint_path, custom_objects={"lr":keras.callbacks.ReduceLROnPlateau}) 
        print(f"Predicted Classess : {(model.predict(test_x) > 0.5).astype('int32')}")
