...
class fashion_mnist_model :
    ...
    def Build_Keras_Model(self):
        ...
        le_cunn_init = tf.keras.initializers.lecun_normal()

        from functools import partial
        RegularizedDense = partial(keras.layers.Dense, activation="selu", use_bias=False, kernel_initializer=le_cunn_init)


        from MCAlphaDropout import MCAlphaDropout
        for i in range(2):
            model.add(MCAlphaDropout(rate=0.2))
    ...
    def fit_model(self, train_X, train_y, X_valid, y_valid, checkpoint_path=None, model_param_path=None, no_epochs=30, batch_size=250, initial_epoch = 0, verbosity=1, get_model = False):
        ...
        callbacks  = callbacks + [lr_perf_scheduler, early_stopping_cb]
