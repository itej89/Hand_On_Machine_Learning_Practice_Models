...
class TextModel :
    ...
    def create_tf_dataset(self, data_set, repeat=None, n_readers=5,
        ...
        # tfDataSet = tfDataSet.shuffle(shuffle_buffer_size) //Can not USE SHUFFLE in STATEFUL RNN
    ...
    def Build_Keras_rnn_Seq_to_Seq_Model(self, batch_size):
        model = keras.models.Sequential([
            keras.layers.GRU(128, stateful=True, return_sequences=True, recurrent_dropout=0.2 ,
            batch_input_shape=[batch_size, None, self.tokenizer.max_id], dropout=0.2),
            keras.layers.GRU(128, stateful=True, return_sequences=True, recurrent_dropout=0.2 ,dropout=0.2),
            #Recurrent Dropout not supported in CUDNN
            # keras.layers.GRU(128, stateful=True, return_sequences=True ,batch_input_shape=[batch_size, None, self.tokenizer.max_id], dropout=0.2, recurrent_dropout=0),
            # keras.layers.GRU(128, stateful=True, return_sequences=True, dropout=0.2, recurrent_dropout=0),
    ...
    def fit_model(self, mode_builder, train, valid, checkpoint_path=None, model_param_path=None, no_epochs=30, initial_epoch = 0, verbosity=1, get_model = False, batch_size=1):
        ...
        callbacks  = callbacks + [lr_decay_callback, ckpt_callback, ResetStatesCallback()]
...
objTextModel.fit_model(objTextModel.Build_Keras_rnn_Seq_to_Seq_Model, train_dataset ,valid_dataset, 
            CheckPointPath, ModleParamtPath, Total_Epochs, Initiate_at_Epoch, 1, batch_size=1)
