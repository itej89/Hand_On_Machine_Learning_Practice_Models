        ...
        for column in data_frame:
            if column != "median_house_value":
                ...
                if data_frame.dtypes[column] == np.float64:
                    ...
                    arr = data_frame[[column]].values
                    np_array = standars_scalar.fit_transform(data_frame[[column]].values)
                    data_frame[[column]] = pd.Series(np_array[0])
        print(data_frame)
...
if __name__ == '__main__':
    ...
    housing = objDownloadData.load_Data(Data_File_Path)