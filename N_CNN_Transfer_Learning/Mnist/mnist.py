...
#region DataProcessing
    ...
    def get_tfds_datasets(self, Fetch_Tuple = False):
        ...
        dataset_train_valid = objLoadData.load_Data_TFDS("mnist", DATASET_PATH_CONSTANTS.GetDirectoryPath(), Fetch_Tuple, "train")
        dataset_train = dataset_train_valid.skip(10000);
        dataset_valid = dataset_train_valid.take(10000);
        dataset_test = objLoadData.load_Data_TFDS("mnist", DATASET_PATH_CONSTANTS.GetDirectoryPath(), Fetch_Tuple, "test")
    ...
    def preprocess(self, serialized_record):
        ...
        print("converting")
        label = tf.one_hot(tf.cast(serialized_record["label"], tf.int32), 10)
        # Recasts it to Float32
        label = tf.cast(label, tf.float32)
        tf.print(serialized_record["label"][0])
        tf.print(label[0])

        return serialized_record["image"], label
    ...
    def create_tfds_train_dataset_streams(self, dataset, repeat=None, n_readers=5,
                            n_read_threads=None, shuffle_buffer_size=10000,
                            n_parse_threads=5, batch_size=32):
        dataset = dataset.repeat(repeat).shuffle(shuffle_buffer_size).batch(batch_size)
    ...
    def fit_model(self, train, valid, checkpoint_path=None, model_param_path=None, no_epochs=30, initial_epoch = 0, verbosity=1, get_model = False):
        ...
        if checkpoint_path is not None:
            ...
            from ResidualUnit import ResidualUnit
            from ResidualUnit import DefaultConv2D
            model = load_model(checkpoint_path, ResidualUnit={'ResidualUnit': ResidualUnit})
            ...
            model = self.Build_Resnet_34_Model()
    ...
    def model_predict(self, checkpoint_path, test_x):
        ...
        optimizer = keras.optimizers.Nadam(0.001, beta_1=0.9, beta_2=0.999)
        model = load_model(checkpoint_path, custom_objects={'ResidualUnit': ResidualUnit, "lr": self.get_lr_metric(optimizer)})
...
model_class = mnist_classifier()

train, test, validation = model_class.get_tfds_datasets(Fetch_Tuple = True)
train_set = model_class.create_tfds_train_dataset_streams(train, repeat=1, batch_size=32)
test_set = model_class.create_tfds_train_dataset_streams(test, repeat=1, batch_size=1)
validation_set = model_class.create_tfds_train_dataset_streams(validation, repeat=1, batch_size=32)
...
# model_class.fit_model(train_set , validation_set, None, None, Total_Epochs, Initiate_at_Epoch, 1)

model_class.model_predict(CheckPointPath, test_set.take(1))
