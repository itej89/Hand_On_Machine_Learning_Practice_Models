...
class RidgeRegularization:
    ...
    #using closed-form solution Eq 4-9 
    def Perform_RidgeRegularization(self, X, y, X_test):
        from sklearn.linear_model import Ridge
        ridge_Reg = Ridge(alpha=1, solver="cholesky")
        ridge_Reg.fit(X,y)
        print(ridge_Reg.predict(X_test))

    #usingSGD
    def Perform_RidgeRegularizationWithSGD(self, X, y, X_test):
        from sklearn.linear_model import SGDRegressor
        sgd_reg = SGDRegressor(penalty="l2")
        sgd_reg.fit(X,y)
        print(sgd_reg.predict(X_test))



ridgeRegularization = RidgeRegularization()
frame_train_x, train_y, frame_test_x, test_y = ridgeRegularization.get_data_sets()
full_pipeline = ridgeRegularization.create_pipeline(frame_train_x)
train_x = ridgeRegularization.runthrough_pipeline(full_pipeline, frame_train_x)
test_x = ridgeRegularization.runthrough_pipeline(full_pipeline, frame_test_x)

feature_list = ridgeRegularization.get_all_features(full_pipeline, frame_train_x)
...
lin_reg = ridgeRegularization.Perform_RidgeRegularizationWithSGD(train_x, train_y, test_x)