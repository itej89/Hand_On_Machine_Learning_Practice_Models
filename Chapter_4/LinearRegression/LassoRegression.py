...
class LassoRegularization:
    ...
    #using closed-form solution to Eq 4-10
    def Perform_LassoRegularization(self, X, y, X_test):
        from sklearn.linear_model import Lasso
        lasso_Reg = Lasso(alpha=0.1)
        lasso_Reg.fit(X,y)
        print(lasso_Reg.predict(X_test))

    #usingSGD
    def Perform_LassoRegularizationWithSGD(self, X, y, X_test):
...
frame_train_x, train_y, frame_test_x, test_y = lassoRegularization.get_data_sets()
full_pipeline = lassoRegularization.create_pipeline(frame_train_x)
train_x = lassoRegularization.runthrough_pipeline(full_pipeline, frame_train_x)
test_x = lassoRegularization.runthrough_pipeline(full_pipeline, frame_test_x)

feature_list = lassoRegularization.get_all_features(full_pipeline, frame_train_x)
...
lin_reg = lassoRegularization.Perform_LassoRegularization(train_x, train_y, test_x)