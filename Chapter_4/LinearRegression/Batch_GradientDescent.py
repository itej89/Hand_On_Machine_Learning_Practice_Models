...
class BatchGRD:
    ...
    def Perform_BatchGRD(self, X, y):
        ...
        for iteration in range(n_iterations):
            gradients = 2/m * X.T.dot(X.dot(theta) - np.array(y).reshape(len(y),1))
...
batchGRD = BatchGRD()
frame_train_x, train_y, frame_test_x, test_y = batchGRD.get_data_sets()
full_pipeline = batchGRD.create_pipeline(frame_train_x)
train_x = batchGRD.runthrough_pipeline(full_pipeline, frame_train_x)
test_x = batchGRD.runthrough_pipeline(full_pipeline, frame_test_x)

feature_list = batchGRD.get_all_features(full_pipeline, frame_train_x)
...
lin_reg = batchGRD.Perform_BatchGRD(train_x, train_y)