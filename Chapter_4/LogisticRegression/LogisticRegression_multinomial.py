...
class LogisticRegression_Iris :
    ...
    def Predict_VerginicaPetalBasedDetection(self, model, X_test, y_test):
        ...
        versicolor_indices = np.where(y_proba[:,1] > 0.5)[0]
        X_test_versicolor = X_test[versicolor_indices]
        X_test_versicolor = X_test_versicolor.T

        setosa_indices = np.where(y_proba[:,2] > 0.5)[0]
        X_test_setosa = X_test[setosa_indices]
        X_test_setosa = X_test_setosa.T

        import matplotlib.pyplot as plt
        plt.scatter(x=X_test_verginica[0], y=X_test_verginica[1], marker='.', label="Setosa")
        plt.scatter( x=X_test_versicolor[0], y=X_test_versicolor[1], marker=',', label="Versicolor")
        plt.scatter( x=X_test_setosa[0], y=X_test_setosa[1], marker='^', label="Verginica")
        plt.show()
...
print("\n")
print(f"Train set x length : %d || Train set y length : %d", len(frame_train_x_verginica__Petal_width), len(train_y))
print("\n")
# print(f"Test  set x length : %d || Test  set y length : %d", len(test_x), len(test_y))
print("\n")

log_Reg = logisticRegression_Iris.Perform_VerginicaDetection(frame_train_x_verginica__Petal_width, train_y)
...
logisticRegression_Iris.Predict_VerginicaPetalBasedDetection(log_Reg, frame_train_x_verginica__Petal_width, test_y)
